...
//
// Many people starting out get confused by this and they end up changing the
// variable, e.g.
//
// static const char col_cyan[]        = "#FF0000"; // it's red
//
// So now col_cyan no longer has the color of cyan, and both the bar background
// and the window border for the selected client will be red because the
// variable is used in two places.
//
// I suppose the idea is that you create your own variables and substitute them
// in the scheme as you want, e.g.
//
...
//
// or if you do not want to use variables then you can just add the colors
// directly, e.g.
//
...
static const Rule rules[] = {
    ...
    // Scratchpad
    // { "Lxtask",  NULL,       NULL,       SCRATCHPAD_MASK_1,            0, 1,
    // -1 }, { "Thunar",  NULL,       NULL,       SCRATCHPAD_MASK_2, 0, 1, -1 },
    // { "Emacs",  NULL,       NULL,       SCRATCHPAD_MASK_3,            0, 1,
    // -1 }, { "obsidian",  NULL,       NULL,       SCRATCHPAD_MASK_4, 0, 1, -1
    // },
};
...
// static const char *dmenu_bluetooth[] = {"dblue -b -fn 'curie' -nb '#2E3440'
// -sf '#282828' -sb '#81A1C1' -nf '#bebebe'", NULL}; Media def
//
// playprev and playnext not working ;; Todo
// static const char *playprev[]               = { "/usr/bin/dbus-send
// --print-reply --dest=org.mpris.MediaPlayer2.playerctld
// /org/mpris/MediaPlayer2 org.mpris.MediaPlayer2.Player.Previous", NULL };
// static const char *playnext[]               = { "/usr/bin/dbus-send
// --print-reply --dest=org.mpris.MediaPlayer2.playerctld
...
// static const char *runspotify[]               = {
// "LD_PRELOAD=/usr/local/lib/spotify-adblock.so spotify", NULL };
...
static Key keys[] = {
    ...
    // Nord BLuetooth
    // {MODKEY2, XK_b, spawn,
    //  SHCMD("dblue -fn 'curie' -nb '#2E3440' -sf '#282828' -sb '#81A1C1' -nf "
    //        "'#bebebe'")},
    ...
    TAGKEYS(XK_1, 0) TAGKEYS(XK_2, 1) TAGKEYS(XK_3, 2) TAGKEYS(XK_4, 3)
        TAGKEYS(XK_5, 4) TAGKEYS(XK_6, 5) TAGKEYS(XK_7, 6) TAGKEYS(XK_8, 7)
            TAGKEYS(XK_9,
                    8){MODKEY | ShiftMask, XK_e, spawn,
                       SHCMD("pkill dwm")}, // WindowsKey + Shift + e - kill dwm
    ...
    // WindowsKey+Shift+v - Run spotify
    {MODKEY | ShiftMask, XK_v, spawn,
     SHCMD("spotify")}, // WindowsKey+Shift+v - Run spotify
    {MODKEY2, XK_space, spawn, {.v = playpause}}, // Alt + Space - Play / Pause
    {MODKEY2, XK_period, spawn,
     SHCMD("/usr/bin/dbus-send --print-reply "
           "--dest=org.mpris.MediaPlayer2.playerctld /org/mpris/MediaPlayer2 "
           "org.mpris.MediaPlayer2.Player.Next")}, // Alt + . - Next
    {MODKEY2, XK_comma, spawn,
     SHCMD("/usr/bin/dbus-send --print-reply "
           "--dest=org.mpris.MediaPlayer2.playerctld /org/mpris/MediaPlayer2 "
           "org.mpris.MediaPlayer2.Player.Previous")}, // Alt + , - Previous
                                                       // Scripts etc
    {MODKEY2, XK_q, spawn, SHCMD("books")},            // Alt + q - Rofi Books
    {MODKEY2, XK_z, spawn, SHCMD("screenshot")},       // Alt + z - screenshot
    {MODKEY, XK_z, spawn,
     SHCMD("rofi -no-lazy-grab -show drun -modi drun")}, // WindowsKey + z -
                                                         // Rofi App Menu
    {MODKEY2, XK_Return, spawn,
     SHCMD("kitty --single-instance")}, // Alt + Enter - kitty
    {MODKEY2 | ShiftMask, XK_l, spawn,
     SHCMD("betterlockscreen -l")},           // Alt + l - betterlockscreen lock
    ...
    {MODKEY2, XK_n, spawn,
     SHCMD("networkmanager_dmenu")}, // Alt + n - Dmenu network manager
