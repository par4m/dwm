...
struct NumTags {
  char limitexceeded[LENGTH(tags) > 27 ? -1 : 1];
};
...
void applyrules(Client *c) {
  ...
  if (c->tags != SCRATCHPAD_MASK_1 && c->tags != SCRATCHPAD_MASK_2 &&
      c->tags != SCRATCHPAD_MASK_3 && c->tags != SCRATCHPAD_MASK_4) {
    c->tags =
...
void bartabdraw(Monitor *m, Client *c, int unused, int x, int w,
                int groupactive) {
  if (!c)
    return;
  int i, nclienttags = 0, nviewtags = 0;

  drw_setscheme(drw, scheme[m->sel == c ? SchemeSel
    ...
    for (i = 0; i < LENGTH(tags); i++) {
      drw_rect(drw,
               (x + w - 2 - ((LENGTH(tags) / BARTAB_TAGSROWS) * BARTAB_TAGSPX) -
                ...
                ((i % (LENGTH(tags) / BARTAB_TAGSROWS)) * BARTAB_TAGSPX)),
...
void bartabcalculate(Monitor *m, int offx, int sw, int passx,
                     void (*tabfn)(Monitor *, Client *, int, int, int, int)) {
  Client *c;
  int i, clientsnmaster = 0, clientsnstack = 0, clientsnfloating = 0,
         masteractive = 0, fulllayout = 0, floatlayout = 0, x, w, tgactive;
  ...
  for (i = 0; i < LENGTH(bartabfloatfns); i++)
    if (m->lt[m->sellt]->arrange == bartabfloatfns[i]) {
      floatlayout = 1;
      break;
    }
  ...
  for (c = m->clients, i = 0; c; c = c->next) {
    ...
    if (clientsnmaster + clientsnstack == 0 || floatlayout) {
      x = offx + (((m->mw - offx - sw) /
                   (clientsnmaster + clientsnstack + clientsnfloating)) *
                  i);
      w = (m->mw - offx - sw) /
          (clientsnmaster + clientsnstack + clientsnfloating);
      tgactive = 1;
    } else if (!c->isfloating &&
               (fulllayout || ((clientsnmaster == 0) ^ (clientsnstack == 0)))) {
      ...
      x = (m->mw * m->mfact) +
          ((((m->mw * (1 - m->mfact)) - sw) / clientsnstack) *
           (i - m->nmaster));
...
void drawbar(Monitor *m) {
  ...
  if (m == selmon || 1) { /* status is only drawn on selected monitor */
    ...
    tw = TEXTW(stext) - lrpad / 2 + 2 +
         getsystraywidth(); /* 2px extra right padding */
